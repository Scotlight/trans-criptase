...
> **同时支持 Codex CLI。** transcriptase 也会读 Codex 的 `~/.codex/sessions/**/rollout-*.jsonl` 日志。两种来源按文件自动探测，**按真实工作目录合并进同一个项目索引**——所以在某项目里检索或续接时，Claude Code 与 Codex 的历史会并排出现，Codex 命中标 `[codex]`。零配置：本机有 `~/.codex` 就自动接入。
...
**Q: 跨平台？**
核心（MCP/CLI）纯 Node，三平台通用。Codex 会话同理：发现、解析、索引、检索全走同一条纯 Node 路径（`~/.codex/sessions` 经 `os.homedir()` 定位，无硬编码分隔符或 `.exe`）。`scan-transcript.ps1` 需要 pwsh，是兜底脚本——MCP 的 `trans_scan` 是它的跨平台等价物。
